...
import com.knowdata.framework.study.fta.utils.CalculationUtils;
...
public class TopEvent extends IntermediateEventDTO {
    ...
    public void createParams() {
        ...
        if (CollectionUtils.isNotEmpty(baseEventList)) {
            ...
            params = Flux.fromIterable(envets).map(item -> new EventParams(item.getName(), CalculationUtils.PARAM_PREFIX + atomicInteger.getAndIncrement(), item.getProbability())).collectList().block();
        ...
        if (CollectionUtils.isNotEmpty(andBaseEvents)) {
            for (BaseEventDTO event : andBaseEvents) {
                if (!(event instanceof IntermediateEventDTO)) {
                    ...
                    builder.append(paramsMap.get(name)).append(CalculationUtils.MULTIPLY);
                } else {
                    builder.append(expressionRecursive(event));
                }
            }
            builder.delete(builder.lastIndexOf(CalculationUtils.MULTIPLY), builder.length());
        } else if (CollectionUtils.isNotEmpty(orBaseEvents)) {
            for (BaseEventDTO event : orBaseEvents) {
                if (!(event instanceof IntermediateEventDTO)) {
                    String name = event.getName();
                    builder.append(paramsMap.get(name)).append(CalculationUtils.ADD);
                } else {
                    builder.append(expressionRecursive(event)).append(CalculationUtils.ADD);
                }
            }
            builder.delete(builder.lastIndexOf(CalculationUtils.ADD), builder.length());
        } else if (CollectionUtils.isNotEmpty(noBaseEvents)) {
    ...
    private void expressionResolver() {
        List<String> list = Arrays.stream(expression.split(CalculationUtils.EMPTY)).collect(Collectors.toList());
        ...
        StringBuilder builder = new StringBuilder();
        calculations = calculations.stream().filter(item -> !Objects.equals(CalculationUtils.ADD, item)).collect(Collectors.toList());
        builder.append("1-");
        for (String calculation : calculations) {
            builder.append(CalculationUtils.LEFT_PARENTHESIS).append("1-").append(calculation).append(CalculationUtils.RIGHT_PARENTHESIS).append(CalculationUtils.MULTIPLY);
        }
        builder.delete(builder.lastIndexOf(CalculationUtils.MULTIPLY), builder.length());
        expression = builder.toString();
    ...
    public String expressionRecursive(BaseEventDTO eventDTO) {
        StringBuilder builder = new StringBuilder();
        builder.append(CalculationUtils.LEFT_PARENTHESIS);
            ...
            if (CollectionUtils.isNotEmpty(e.getAndBaseEvents())) {
                for (BaseEventDTO event : e.getAndBaseEvents()) {
                    if (!(event instanceof IntermediateEventDTO)) {
                        ...
                        builder.append(expressionRecursive(event)).append(CalculationUtils.MULTIPLY);
                    }
                }
                builder.delete(builder.lastIndexOf(CalculationUtils.MULTIPLY), builder.length());
        ...
        builder.append(CalculationUtils.RIGHT_PARENTHESIS);
